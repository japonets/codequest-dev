...
const drawSun = (ctx: CanvasRenderingContext2D, centerX: number, centerY: number, radius: number, image: HTMLImageElement) => {
    ...
    ctx.drawImage(image, centerX - radius*3.5, centerY - radius*3.5, radius * 7, radius * 7)
}

const drawPlanets = (ctx: CanvasRenderingContext2D, planetPosition: PlanetPosition[], angles: number[], centerX: number, centerY: number, tema: string) => {
    ...
    planetPosition.forEach((planet, index) => { //desenha cada planeta na posição calculada
        ...
        if (tema === 'light') {
            ctx.fillStyle = planet.lightColor; //cor do planeta no tema claro
        }else{
            ctx.fillStyle = planet.color; //cor do planeta no tema escuro
        }
        ctx.fill();
    });
}

const drawOrbits = (ctx: CanvasRenderingContext2D, planets: Planet[], centerX: number, centerY: number, tema: string) => {

    planets.forEach(planet => {
        ctx.beginPath();
        ctx.arc(centerX, centerY, planet.orbitRadius, 0, Math.PI * 2); //desenha a órbita do planeta
        if (tema === 'light') {
            ctx.strokeStyle = 'black'; //cor das órbitas
        }else{
...
const drawStars = (ctx: CanvasRenderingContext2D, stars: Star[], shinyStars: ShinyStar[], tema: string) => {

    stars.forEach(star => {
        ctx.beginPath();
        ctx.arc(star.x, star.y, star.radius, 0, Math.PI * 2);
        if (tema === 'light') {
            ctx.fillStyle = `rgba(0, 0, 0, ${star.opacity})`; //estrelas pretas no tema claro
        }else{
            ctx.fillStyle = `rgba(255, 255, 255, ${star.opacity})`; //estrelas brancas no tema escuro
        }
        ctx.fill();
    });
